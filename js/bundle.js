...
function calc() {
  ...
  let sex, height, weight, age, ratio; // Если в локал сторадже есть какая то информация, то мы берем и помещаем ее в sex, если нет, то установим ее по умолчанию с заданными данными
  ...
  calcTotal(); //  Функция по получению данных из статических элементов верстки (интенсивность активности)
  ...
  getStaticInformation('.calculating__choose_big div', 'calculating__choose-item_active'); //  Функция по получению данных из динамических элементов верстки (инпутов)
...
function cards() {
  ...
  class MenuCard {
    ...
    render() {
      // нужно создать Элемент, в него поместиьт верстку, дополнить данными которые приходят как элементы и поместить элемент на страницу
  ...
  function bindpostData(form) {
    form.addEventListener('submit', e => {
      ...
      statusMessage.style.cssText = `
                ...
                margin: 0 auto;
            `;
      form.insertAdjacentElement('afterend', statusMessage); // Трансформация форм дате в json формат

      const formData = new FormData(form); // Берем formData превращаем ее в массив массивов formData.entries(), для того чтобы могли нормально работать с ней, после этого преращаем ее в классический объект (Object.fromEntries(formData.entries())) а после превращаем в json
...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, activeClass) {
  ...
  function showTabContent() {
    ...
    // если функция вызывается без аргумента, то по умолчанию всегда будет 0
  ...
  hideTabContent();
  showTabContent(); // 0 - дефолтное значение всегда в данном случае
...
const getResource = async url => {
  const res = await fetch(url); // Если в запросе что-то пошло не так, то я выкину ошибку. Если выкинуть ошибку в ручном режиме то сработает блок кода .catch()
...
(__webpack_require__(/*! es6-promise */ "./node_modules/es6-promise/dist/es6-promise.js").polyfill)();
 ...
 // Объединяем файлы, заимпортировав их в главный
...
document.addEventListener('DOMContentLoaded', () => {
  // ЧТОБЫ МОДАЛЬНОЕ ОКНО ПОЯВЛЯЛОСЬ ЧЕРЕЗ ОПРЕДЕЛЕННЫЙ ПРОМЕЖУТОК ВРЕМЕНи
  const modalTimerId = setTimeout(() => (0,_modules_modal__WEBPACK_IMPORTED_MODULE_2__.openModal)('.modal', modalTimerId), 50000); // Вызываем эти функции
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_3__["default"])('.timer', '2022-10-25');
  ...
  }); // в конце после сборки вебпаком не забыть изменить пути в индекс html со script.js на bundle.js
}); // После всего не забыть вызвать вебпак чтобы модули собрались
}();
/******/ })()
;
